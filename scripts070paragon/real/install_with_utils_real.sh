...
make PETSC_DIR=$PETSC_DIR PETSC_ARCH=linux-gnu-real-32 ${MAKEFLAGS} all

cd src/binding/petsc4py 
PETSC_ARCH=linux-gnu-real-32 pip3 install --no-cache-dir --no-dependencies . 
...
cd ${SLEPC_DIR} 
PETSC_ARCH=linux-gnu-real-32 python3 ./configure 
make SLEPC_DIR=${SLEPC_DIR} PETSC_DIR=$PETSC_DIR PETSC_ARCH=linux-gnu-real-32
...
# real build
export DOLFINX_VERSION=v0.7.1
export DOLFINX_CMAKE_BUILD_TYPE="RelWithDebInfo"

git clone -b ${DOLFINX_VERSION} --single-branch https://github.com/FEniCS/dolfinx.git
cd dolfinx 
mkdir -p build-real && cd build-real
PETSC_ARCH=linux-gnu-real-32 cmake -G Ninja -DCMAKE_INSTALL_PREFIX=$INSTALL_DIR/dolfinx-real -DCMAKE_BUILD_TYPE=${DOLFINX_CMAKE_BUILD_TYPE} -DCMAKE_CXX_FLAGS=${DOLFINX_CMAKE_CXX_FLAGS} ../cpp
ninja install
cd ../python
export CMAKE_PREFIX_PATH=$INSTALL_DIR/dolfinx-real:$CMAKE_PREFIX_PATH

CXXFLAGS=${DOLFINX_CMAKE_CXX_FLAGS} PETSC_ARCH=linux-gnu-real-32 pip3 install --target $VENV_DIR/lib/python3.8/dist-packages --no-dependencies --no-cache-dir .
cd ../.. 
