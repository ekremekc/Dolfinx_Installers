...
export WORKING_DIR=$HOME/Dev/Venvs
export VENV_NAME=v060
...
export cmake_version_required=3.20.0
...
python -m pip install --no-cache-dir Cython==0.29.32
pip3 install wheel
...
# We set the new compiler variables

export CC=$HOME/gcc-12/bin/gcc
export CXX=$HOME/gcc-12/bin/g++ 
export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:$HOME/gcc-12/lib64
...
cd $SRC_DIR
# python3.8 -m pip install --upgrade --force-reinstall setuptools 
...
CXXFLAGS=${DOLFINX_CMAKE_CXX_FLAGS} PETSC_ARCH=linux-gnu-complex-32 pip3 install -v --target $VENV_DIR/lib/python3.8/dist-packages --no-dependencies .
cd $SRC_DIR
...
export fname="activate.sh"
echo "export VENV_DIR=/home/ee331/Dev/Venvs/v060" > $SCRIPT_DIR/$fname
echo "source \$VENV_DIR/bin/activate" >> $SCRIPT_DIR/$fname
echo "export CC=\$HOME/gcc-12/bin/gcc" >> $SCRIPT_DIR/$fname
echo "export CXX=\$HOME/gcc-12/bin/g++" >> $SCRIPT_DIR/$fname
echo "export LD_LIBRARY_PATH=\$HOME/gcc-12/lib64:\$LD_LIBRARY_PATH" >> $SCRIPT_DIR/$fname
echo "export OMPI=\$VENV_DIR/installation/openmpi" >> $SCRIPT_DIR/$fname
echo "export PATH=\$OMPI/bin:\$PATH" >> $SCRIPT_DIR/$fname
echo "export LD_LIBRARY_PATH=\$OMPI/lib:\$LD_LIBRARY_PATH" >> $SCRIPT_DIR/$fname
echo "export KAHIP=\$VENV_DIR/installation/KaHIP" >> $SCRIPT_DIR/$fname
echo "export PATH=\$KAHIP/bin:\$PATH" >> $SCRIPT_DIR/$fname
echo "export LD_LIBRARY_PATH=\$LD_LIBRARY_PATH:\$KAHIP/lib" >> $SCRIPT_DIR/$fname
echo "export ADIOS2=\$VENV_DIR/installation/adios2" >> $SCRIPT_DIR/$fname
echo "export PATH=\$ADIOS2/bin:\$PATH" >> $SCRIPT_DIR/$fname
echo "export LD_LIBRARY_PATH=\$ADIOS2/lib:\$LD_LIBRARY_PATH" >> $SCRIPT_DIR/$fname
echo "export PKG_CONFIG_PATH=\$VENV_DIR/installation/dolfinx-complex/lib/pkgconfig:\$PKG_CONFIG_PATH" >> $SCRIPT_DIR/$fname
echo "export PETSC_ARCH=linux-gnu-complex-32" >> $SCRIPT_DIR/$fname
echo "export PYTHONPATH=\$VENV_DIR/lib/python3.8/dist-packages:/usr/bin/python3:\$PYTHONPATH" >> $SCRIPT_DIR/$fname
echo "export LD_LIBRARY_PATH=\$VENV_DIR/installation/lib:\$LD_LIBRARY_PATH" >> $SCRIPT_DIR/$fname
